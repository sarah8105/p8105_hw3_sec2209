...
In the next code chunk, I create a single-panel plot that displays the 24-hour activity trends for each day, color-coded by day of the week. I've displayed these data using `geo_hex()` to limit the amount of data presented. 
...
Based on this graph, this patient begins increasing their activity level around 6 AM. In general, this patient's activity level remains relatively steady throughout the day, with some noticeable exceptions. For example, on Sundays there is an increase in activity around 12 PM. During weekdays and Saturdays, there is an increase in activity around 8-9 PM.
...
The resulting data set contains `r nrow(ny_noaa)` rows  and `r ncol(ny_noaa)` columns. There is substantial missing data, with `r ny_noaa %>% summarize(mean(is.na(tmax))) * 100 %>% round(1)`% of observations missing maximum temperature, `r ny_noaa %>% summarize(mean(is.na(tmin))) * 100 %>% round(1)`% of observations missing minimum temperature, `r ny_noaa %>% summarize(mean(is.na(snow))) * 100 %>% round(1)`% of observations missing snow amount, and `r ny_noaa %>% summarize(mean(is.na(snwd))) * 100 %>% round(1)`% of observations missing snow depth. Since it rarely snows in New York outside of winter months, the most common observed snowfall values is`r summarize(ny_noaa, mode(snwd))` orders,
...
```{r noaa_plot1}
noaa %>%
  select(id, month_name, tmax) %>%
  filter(month_name %in% c("January", "July")) %>%
  group_by(id, month_name) %>%
  summarize(mean_tmax = mean(tmax, na.rm = TRUE)) %>%
  drop_na(mean_tmax) %>% 
  ggplot(aes(x = mean_tmax, color = month_name)) + 
    geom_histogram(aes(fill = month_name), alpha = .5) +
...
From this graph, we can see that average maximum temperatures in July are much higher than temperatures in January, as expected. There is also a wider range and more outliers in the January temperatures across NY NOAA stations than in the July temperatures. 
